...
Dependence QC reports Pearson, Spearman, valid spot count, undefined fraction, seeded permutation p-value, and Benjamini-Hochberg FDR when at least two explicit paired-pole axes exist. High dependence is a warning only. Axes are never PCA-transformed, orthogonalized, removed, or reweighted. The pan-cancer/default axis weight remains 1.0.
...
The **v0.65 Reliability** result tab explicitly labels **Legacy signed Balance** and **Nonnegative Activity/Co-activation**. It shows Pre/Post summaries, valid counts/fractions, QC reasons, gene audit/coverage, axis dependence, and a Score domain diagnostic tab. The Figure tab exposes reliability QC figures.
...
Development validation on 2026-08-19 used samples 30, 38, 41, 42, 43, and 44 (Pre/Post; 12 H5AD files). Input SHA-256 values matched the preserved v0.6 H/V validation metadata 12/12.
...
- Fifteen reliability CSV/JSON/PNG artifacts were byte-identical in a repeated seed-42 run.
- The run produced 7,391 spot rows, 48 coverage rows, and 36 exclusivity audit rows with zero hard overlaps.
- The pre-z-score Activity source was finite and nonnegative for 100% of spots in all 12 slides, so all six pairs passed the count-and-fraction gate. Direction/CA-fraction remained undefined at zero-Activity spots as required; direction-defined fractions ranged from 48.97% to 97.29%.
